...
$delete_id = $_GET["wishlist_id"];
$delete_cart = $mysql->prepare("delete from wishlists where wishlist_id = ? and user_id = ? ");
...
<div class="container">
    ...
    <table class="table table-hover">
        ...
        <tbody>
            <?php
                $i = 0;
                $cart_query = $mysql->query("select * from wishlists where user_id = '$user_id';");
                while($row = $cart_query->fetch_assoc()){
                    $i+=1;
                    $wishlist_id = $row["wishlist_id"];
                    $wishlist_product_name = $row["product_name"];
                    $wishlist_product_rate = $row["product_rate"];

                    echo "<tr>";
                    echo "<td>$i</td>";
                    echo "<td>$wishlist_product_name</td>";;
                    echo "<td>$wishlist_product_rate</td>";
                    echo "<td><a href='wishlists.php?delete_id=$wishlist_id'>Delete</a></td>";
                    echo "</td>";
                }
